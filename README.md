Esta API permite la conexión de un backend sencillo en java con una base de datos en MondoDB, fue implementada en dos proyectos propios que la usan simultáneamente para la empresa ficticia "ComiCommerce".

Contiene documentación hecha en Swagger y la implementación de HATEOAS.

URL Swagger:
https://mondongonzalo.up.railway.app/swagger-ui/index.html

Esta API fue desplegada en Railway en la siguiente URL:
https://mondongonzalo.up.railway.app

Los endpoints son los siguientes:

GET USUARIOS
https://mondongonzalo.up.railway.app/api/users

GET PRODUCTOS
https://mondongonzalo.up.railway.app/api/products

GET CARRITO DE UN USUARIO
https://mondongonzalo.up.railway.app/carrito/{usuarioId} (Cambiar {usuarioId} por la ID, la web y app lo hacen por defecto)
